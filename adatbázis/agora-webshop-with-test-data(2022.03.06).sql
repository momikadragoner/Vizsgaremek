...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 06, 2022 at 08:42 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteCart` (IN `cartProdId` INT)  BEGIN
	UPDATE cart SET cart.sum_price = (cart.sum_price - (SELECT product.price FROM product WHERE product.product_id = (SELECT cart_product.product_id FROM cart_product WHERE cart_product.cart_product_id = cartProdId))) WHERE cart.cart_id = (SELECT cart.cart_id FROM cart INNER JOIN cart_product ON cart_product.cart_id = cart.cart_id WHERE cart_product.cart_product_id = cartProdId);
...
BEGIN
    ...
    END IF;
    UPDATE cart SET cart.sum_price = cart.sum_price + (SELECT product.price FROM product WHERE product.product_id = prodId) WHERE cart.cart_id = @currCartId;
END$$
...
--
ALTER TABLE `product_material`
  MODIFY `product_material_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- AUTO_INCREMENT for table `product_picture`
--
ALTER TABLE `product_picture`
  MODIFY `product_picture_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=33;

--
-- AUTO_INCREMENT for table `product_tag`
--
ALTER TABLE `product_tag`
  MODIFY `product_tag_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=37;
