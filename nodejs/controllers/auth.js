...
exports.userSignupAsVendor=(req,res,next)=>{
    ...
    try{
        ...
        conn.query(checkEmail,[email],(err,result, fields)=>{
            if(!result.length){
                conn.query(`CALL insertVendor(?,?,?,?,?,?,?,?)`,[lastName, firstName, email, hashed_password, "assets/def-pfp1.png", "assets/default_assets/def-bg3.png", new Date(),1],(err,result, fields)=>{
                    if(err){
                        res.send({status:0, data:err});
                    } else{      
                        conn.query(`insert into vendor_detail(member_id) select member_id from member where email=?`,[email],(err,result,fields)=>{
                            if(err){
                                    console.log(err);
                                    res.send({status:0, data:err});
                            }
